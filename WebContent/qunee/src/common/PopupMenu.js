...
//            dom.ontouchstart = onclick;
    ...
    getMenuItems: function(graph, data, evt){
      ...
      if (data) {
        ...
        if (isShapeNode)
        {
            var isDashLine2 = data.getStyle(Q.Styles.SHAPE_LINE_DASH) || Q.DefaultStyles[Q.Styles.SHAPE_LINE_DASH];
            items.push({
                text: isDashLine2 ? getI18NString('Solid Line') : getI18NString('Dashed Line'), action: function (evt, item) {
                    data.setStyle(Q.Styles.SHAPE_LINE_DASH, isDashLine2 ? null : [5, 3]);
                }
            });
            items.push({
            text: getI18NString('Line Width'), action: function (evt, item) {
              Q.prompt(getI18NString('Input Line Width'), data.getStyle(Q.Styles.SHAPE_STROKE) || Q.DefaultStyles[Q.Styles.SHAPE_STROKE], function (lineWidth) {
                if (lineWidth === null) {
                  return;
                }
                lineWidth = parseFloat(lineWidth);
                data.setStyle(Q.Styles.SHAPE_STROKE, lineWidth);
              })
            }
          });
        }
      ...
      items.push(Q.PopupMenu.Separator);
      items.push({
        text: getI18NString('update'), action: function (evt, item) {
          var localXY = graph.globalToLocal(evt);
          console.log(data)
         updatePoint(data,evt);
        }
      });
      items.push({
        text: getI18NString('delete'), action: function (evt, item) {
          var localXY = graph.globalToLocal(evt);
          // 右键删除某一对象
  ...
  var _contextmenuListener = {
    onstart: function (evt, graph) {
      if (graph._popupmenu != null) graph._popupmenu.hide();
    }
  }
