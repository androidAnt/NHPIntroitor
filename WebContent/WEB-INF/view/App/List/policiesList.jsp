...
<%-- 				<a href="${root}/app/module" class="icon color8 iconfont icon-home_light"></a> --%>
		...
		<div id="mescroll" class="t-c p6 f46 color5 mescroll">
			<div class="policies-content-infoList">
				<ul id="newsList" class="news-list infoNewsList">
<%-- 				  <c:forEach items="${page.data}" var="policies" > --%>
<!-- 					 <li> -->
<%-- 					    <a href="${root}/app/module/policiesInfo?id=${policies.policiesId}"> --%>
<!-- 					        <div class="txt"> -->
<!-- 					            <span class="title"> -->
<%-- 					              ${ policies.policiesName} --%>
<!-- 					            </span> -->
<%-- 								<span class="time">${ policies.creatDate}【MORE】</span> --%>
<!-- 					        </div> -->
<!-- 					    </a> -->
<!-- 					</li> -->
<%-- 					</c:forEach> --%>
		...
		var isShow = '${isShow}';
			...
			window.location.href="${root}/app/module/policiesList?isShow="+isShow;
// 				//加载轮播数据..
// 				//...
// 				//加载列表数据
...
// 				});
			...
			function setListData(data, isAppend) {
				...
				for (var i = 0; i < data.length; i++) {
					var newObj=data[i];
					var str = '<a href="${root}/app/module/policiesInfo?id='+newObj.policiesId+'&isShow='+isShow+'">'
					str+=' <div class="txt">'
					str+='<span class="title">'+newObj.policiesName+'</span>';
					str+='<span class="time">'+newObj.creatDate+'</span>'
					str+=' </div> </a>'
...
//  		                      data.append("menuId", );
								//此处模拟上拉加载返回的数据
								$.ajax({
									url : "${root}/app/module/policiesListAjax",
									type : "post",
									data : data,
									async : false,
									dataType : "JSON",
