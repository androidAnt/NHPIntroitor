...
<%-- 				<a href="${root}/app/module" class="icon color8 iconfont icon-home_light"></a> --%>
		...
		<div id="mescroll"  class="t-c p6 f46 color5 mescroll">
			<div class="menuindex-content-info">
				<ul id="newsList" class="news-list infoNewsList">
<%-- 				<c:forEach items="${page.data}" var="ghMenuManage" > --%>
<!-- 					 <li> -->
<%-- 					    <a href="${root}/app/module/details?id=${ghMenuManage.contentId}&menuId=${menu.menuId}"> --%>
<%-- 					        <div class="img" style="background-image: url(${root}/home/module/downloadById?fileId=${ghMenuManage.fileId})"></div> --%>
<!-- 					        <div class="txt"> -->
<!-- 					            <span class="title"> -->
<%-- 					                ${ghMenuManage.contentTitle} --%>
<!-- 					            </span> -->
<!-- 					            <p> -->
<%-- 					            ${ghMenuManage.summary} --%>
<!-- 					            </p> -->
<%-- 								<span class="time"> ${ghMenuManage.creatDate}【MORE】</span> --%>
<!-- 					        </div> -->
<!-- 					    </a> -->
<!-- 					</li> -->
<%-- 				</c:forEach> --%>
		...
		<aside class="ui-aside asideLift w40 bg-color-primary">
			<ul class="nav-cl">
				...
				<li><a href="${root}/app/module/contentType?id=${ContentType.contentTypeId}&isShow=${isShow}">${ContentType.contentTypeName}</a></li>
				</c:forEach>
			</ul>
		...
		var isShow = '${isShow}';
			...
			var byConditionJson = '${byConditionJson}';
			var conditionObject = eval("(" + byConditionJson + ")");
			var menuJson = '${menuJson}';
			var menuObject = eval("(" + menuJson + ")");
			$(function(){
			//创建MeScroll对象
			...
			function downCallback(){
			 window.location.href="${root}/app/module/contentType?id="+conditionObject.contentTypeId+"&isShow="+isShow;
// 				//加载轮播数据..
// 				//...
// 				//加载列表数据
...
// 				});
			...
			function setListData(data, isAppend) {
				...
				for (var i = 0; i < data.length; i++) {
					var newObj=data[i];
					var str = '<a href="${root}/app/module/details?id='+newObj.contentId+'&menuId='+menuObject.menuId+'&isShow='+isShow+'">'
					str+='<div class="img" style="background-image: url(${root}/home/module/downloadById?fileId='+newObj.fileId+')"></div>'
					str+=' <div class="txt">'
...
// 	                		newArr.push(newObj);
	                	      ...
	                	      var data = new FormData();
 		                      ...
 		                      data.append("id", conditionObject.contentTypeId);
								//此处模拟上拉加载返回的数据
								$.ajax({
									url : "${root}/app/module/contentTypeAjax",
									type : "post",
									data : data,
									async : false,
									dataType : "JSON",
