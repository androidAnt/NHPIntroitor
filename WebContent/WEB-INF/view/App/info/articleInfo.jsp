...
<%-- 				<a href="${root}/app/module" class="icon color8 iconfont icon-home_light"></a> --%>
		...
		<div class="t-c p6 f46 color5">
			<div class="article-info">
				<p class="info-title">${GhContentManage.contentTitle }</p>
				<p class="info-time"> 时间:${GhContentManage.creatDate }</p>
				<c:if test="${not empty GhContentManage.imgFileList}">
				<ul>
				<li><img src="${root}/home/module/downloadById?fileId=${GhContentManage.imgFileList[0].fileId}" class="col-12 radius50 padding1" /></li>
				</ul>
				</c:if>
				<div style="text-align: left;">
				<p  class="info-content"> 
				${GhContentManage.contentDetail }
				</p>
				</div>
				<c:if test="${not empty GhContentManage.contentUrl}">
				<p class="content">内容连接:
				<a class="content-fj-a" href="${GhContentManage.contentUrl}">${GhContentManage.contentUrl}</a></p>
				</c:if>
                <!--附件 -->
				<c:if test="${not empty GhContentManage.fileList}">
				<c:forEach items="${GhContentManage.fileList}" var="file" >
