...
function imgscrool(obj){
	...
	//var Width = document.getElementById("top").clientWidth; 
	...
	$(obj+" .banner .num li").hover(function(){
		...
		i=index;
		//$(obj+" .banner .img").stop().animate({left:-index*sWidth},1000)	
		$(obj+" .banner .img").stop().animate({left:-index*sWidth},'fast')	
...
//处理轮播初始宽度问题	
	...
	$(obj+" .banner .img").width(sWidth*a3+'px');
	...
	function move(){
		...
		// 修改轮播每屏宽度
		if(i==-1){
			$(obj+" .banner .img").css({left:-(size-1)*sWidth})
			i=size-2;
		}	
		$(obj+" .banner .img").stop(true).delay(200).animate({left:-i*sWidth},'fast',function(){
			moving = false;
		})
