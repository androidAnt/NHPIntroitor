...
(function (window, document) {
	...
	Croppic.prototype = {
		...
		init: function () {
			var that = this;
			that.imgMaxSize=that.options.imgMaxSize;
			...
			that.form.find('input[type="file"]').change(function(){
				...
				if(that.options.processInline){			
				    ...
				    if (typeof FileReader == "undefined"){
						if (that.options.onError) that.options.onError.call(that,"你的浏览器不支持上传功能");
						that.reset();
					}else{
						...
						if(file.size>that.imgMaxSize*1024*1024){
							that.hideLoader();
							that.imgUploadControl.show();
							showMes("0","图片不能大于"+that.imgMaxSize+"M");
						}else{
		...
		afterCrop: function (data) {
            ...
            if (response.status == 'error'||response.status=="1") {
                if (that.options.onError) that.options.onError.call(that,response.msg);
				that.hideLoader();
				setTimeout( function(){ that.reset(); },2000)	
            }
